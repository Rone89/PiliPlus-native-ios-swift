...
这是一个用 SwiftUI 重写的 PiliPlus iOS 原生版本首版工程，目标是先交付一个可编译、可打包、可通过 GitHub Actions 直接产出未签名 IPA 的基础版本。
...
`source/` 目录只作为原 Flutter 项目的本地参考，不会参与新工程构建。
...
这个首版优先完成“原生重写 + 可发布”的闭环，所以暂未覆盖 Flutter 原项目的全部高级功能，例如登录、动态、收藏、私信、弹幕、下载等。后续可以在现有原生架构上继续扩展。
